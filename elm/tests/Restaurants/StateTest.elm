...
suite =
  describe "In the State module"
      [ describe "State.update AddRestaurant"
          [ test "updates the empty model with the new restaurant" <|
            \_ ->
                ...
                in
                  Expect.equal ({ restaurantList = [restaurant], newRestaurant = Types.emptyRestaurant }, Cmd.none) (State.update (Types.AddRestaurant restaurant) { restaurantList = [], newRestaurant = Types.emptyRestaurant })
            , test "updates the existing model with the new restaurant" <|
              \_ ->
                  let
                    ...
                    restaurant = {name = "Test Name", url = "Test URL", postedBy = "Test Posted By", postedOn = "Test Posted On"}
                  in
                    Expect.equal ({ restaurantList = [restaurant, existing], newRestaurant = Types.emptyRestaurant }, Cmd.none) (State.update (Types.AddRestaurant restaurant) {restaurantList = [existing], newRestaurant = Types.emptyRestaurant})
          ]
        , describe "State.update UpdateRestaurants"
            [ test "creates a model with the returned list when successful" <|
              \_ ->
                  ...
                  in
                    Expect.equal ({ restaurantList = [restaurant], newRestaurant = Types.emptyRestaurant }, Cmd.none) (State.update (Types.UpdateRestaurants result) { restaurantList = [], newRestaurant = Types.emptyRestaurant } )
              , test "keeps existing model when unsuccessful" <|
                \_ ->
                    let
                      existing = { restaurantList = [ { name = "Test", url = "Test", postedBy = "Test", postedOn = "Test" } ], newRestaurant = Types.emptyRestaurant }
